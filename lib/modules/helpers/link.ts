...
export async function CreateLink({
  ...
  let checkExists = await PostgresClient.any(
    "SELECT code, target FROM links WHERE code = $1",
    code
  );
  if (checkExists.length > 0) throw "code_already_exists";

  // Create the link in the database
  await PostgresClient.none("INSERT INTO links(code, target) VALUES ($1, $2)", [
    code,
    target,
  ]);
...
export async function GetLink(
  code: string
): Promise<{ code: string; target: string; uses: number }> {
  // Check if the code currently exists
...
export async function GetAllLinks(): Promise<
  { code: string; target: string; uses: number }[]
> {
  ...
  let checkExists: {
    ...
    target: string;
    uses: number;
  }[] = await PostgresClient.many("SELECT code, target, uses FROM links");
...
export async function IncrementVisitCount(
  ...
  let currentVisitCount: number = Number(
    await RedisClient.get(`route/${code}/visits`)
  );
  if (!currentVisitCount)
    currentVisitCount = await PostgresClient.oneOrNone(
      "SELECT uses FROM links WHERE code = $1",
      [code]
    );
  ...
  if (!getIPFromRedis) {
    ...
    await PostgresClient.none("UPDATE links SET uses = $1 WHERE code = $2", [
      currentVisitCount,
      code,
    ]);
  }
